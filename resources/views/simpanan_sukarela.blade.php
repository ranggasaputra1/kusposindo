...
<html lang="en">
 ...
 <link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />
    <title>KUSPOSINDO | Simpanan Sukarela</title>
  </head>
  <body id="page-top">
  ...
  <div class="dropdown-menu" aria-labelledby="dropdownMenu2">
    ...
    <a href="simpanan_sukarela" style="color: black;"><button class="dropdown-item active" type="button" style="background-color:#e67e22">Sukarela</button></a>
  </div>
</div>
            <li class="nav-item">
...
<!-- judul  -->
                    <div class="container-fluid px-4">
                                <h1 class="mt-4 container " style="text-align:left">Anggota | Simpanan Sukarela</h1>
                ...
                <div class="modal-dialog" role="document">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLongTitle">Tambah Data Simpanan Sukarela</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                        </button>
                        </div>
                        <div class="modal-body">
                <form method="post" action="simpanan_sukarela/add">
                {{ csrf_field() }}
                <div class="form-group has-warning"><label >ID simpanan Sukarela</label>
                    <input type="number" class="form-control" name="ID_simpanan_sukarela" placeholder="Sudah Diisi Otomatis" readonly>
                    </div>
                  ...
                  <select class="form-control" id="search_anggota" name="ID_anggota" data-width="100%">
                    ...
                    <input type="date" class="form-control" name="tanggal_bayar_sukarela" required>
                    </div>

                    <div class="form-group has-warning"><label >Nominal</label>
                    <input type="text" class="form-control" name="nominal_sukarela" required>
                    </div>

                    <div class="form-group has-warning"><label >Jangka waktu</label>
                    <input type="text" class="form-control" name="jangka_waktu_sukarela" required>
                    </div>

                    <div class="form-group has-warning"><label >Jumlah</label>
                    <input type="number" class="form-control" name="jumlah_sukarela" required>
                    </div>
     ...
     @endif
                         <!-- tabel  -->
                                    ...
                                    <div class="card-header">
                                        ...
                                        DataTable Simpanan Sukarela | KUSPOSINDO
                                    </div>
                                    <div class="card-body">
                    ...
                    @foreach($simpanan_sukarelas as $ss)
                    <tr style="text-transform: uppercase">
                    ...
                    <td style="text-align: center;">{{$ss->jumlah_sukarela}}</td>
                      <td> <center>
                      <button type="button" class="btn btn-outline-primary" data-toggle="modal" data-target="#ubahsimpanan_sukarela{{$ss->ID_simpanan_sukarela}}">
                         Edit</button> </center>
                        <div class="modal fade" id="ubahsimpanan_sukarela{{$ss->ID_simpanan_sukarela}}">
                        <div class="modal-dialog">
                            <div class="modal-content">
                              <div class="modal-header">

                                  <h4 class="modal-title">Masukan Data Baru Simpanan Sukarela</h4>
                                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                  </button>
                                </div>
                                <div class="modal-body" style="text-align: left">
                                  <form method="post" action="simpanan_sukarela/update">
                                  @csrf
                                  <div class="form-group has-warning"><label >ID simpanan Sukarela</label>
                    <input type="number" class="form-control" name="ID_simpanan_sukarela" value="{{$ss->ID_simpanan_sukarela}}" placeholder="Sudah Diisi Otomatis" readonly>
                    </div>
                    
                    <div class="form-group has-warning"><label >Anggota</label>
                            <input type="text" class="form-control" name="ID_anggota" value="{{$ss->ID_anggota}}" readonly>
                            </div>

                    <div class="form-group has-warning"><label >Tanggal Bayar</label>
                    <input type="date" class="form-control" name="tanggal_bayar_sukarela" value="{{$ss->tanggal_bayar_sukarela}}" required>
                    </div>

                    <div class="form-group has-warning"><label >Nominal</label>
                    <input type="text" class="form-control" name="nominal_sukarela" value="{{$ss->nominal_sukarela}}" required>
                    </div>

                    <div class="form-group has-warning"><label >Jangka waktu</label>
                    <input type="text" class="form-control" name="jangka_waktu_sukarela" value="{{$ss->jangka_waktu_sukarela}}" required>
                    </div>

                    <div class="form-group has-warning"><label >Jumlah</label>
                    <input type="number" class="form-control" name="jumlah_sukarela" value="{{$ss->jumlah_sukarela}}" required>
                    </div><br>
                    <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                <button type="submit" class="btn btn-primary" onclick="return confirm('Apa Anda Yakin Akan Memperbarui Data?')">Save</button>
                      ...
                      <td> </center>
                      <a href="simpanan_sukarela/hapus/{{$ss->ID_simpanan_sukarela}}" type="button " class="btn btn-outline-danger" onclick="return confirm('Apa Anda Yakin Akan Menghapus Data?')" style="text-transform: none">
                    Hapus</i></a></center>
                        </td>
                    </tr>
                    @endforeach
